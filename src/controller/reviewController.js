...
const createReview = async function (req, res) {
  try {
    ...
    if (!review)
      return res
        .status(400)
        .send({ status: false, message: "reviews is mandatory" });

    if (!isValidName(review))
      return res
        .status(400)
        .send({ status: false, message: "reviews should be string !!" });
...
const updateReview = async function (req, res) {
  try {
    ...
    const existBook = await bookModel
      .findOne({ _id: book, isDeleted: false })
      .lean();
    if (!existBook) {
    ...
    if (!isValidBody(requestBody)) {
      return res
        .status(400)
        .send({ status: false, message: "required some mandatory data" });
    }
    ...
    if (reviewedBy !== undefined) {
      ...
      filteredData["reviewedBy"] = reviewedBy
        .trim()
        .split(" ")
        .filter((a) => a)
        .join(" ");
    ...
    if (updateReview) {
      ...
      return res
        .status(200)
        .send({ status: true, message: "Success", data: existBook });
    }
  } catch (err) {
